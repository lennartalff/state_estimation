...
void Interface::SetImuData(const ImuSample &imu_sample) {
  ...
  if (imu_updated_) {
    ...
    imu_sample_delayed = imu_buffer_.Oldest();
    min_observation_interval_us =
        (imu_sample.time_us - imu_sample_delayed.time_us) /
...
void Interface::SetVisionData(const VisionSample &vision_sample) {
    ...
    if ((vision_sample.time_us - time_last_vision_) > min_observation_interval_us) {
...
bool Interface::InitInterface(uint64_t timestamp_us) {
  ...
  observation_buffer_length_ =
      filter_delay_us / settings_.min_observation_interval_us;
  observation_buffer_length_ =
  ...
  imu_sample_delayed.time_us = timestamp_us;
  return true;
}
